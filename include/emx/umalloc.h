...
#if defined (__MT__)
#define _UM_DEFAULT_REGULAR_HEAP tp->_th_rheap
#define _UM_DEFAULT_TILED_HEAP   tp->_th_theap
#define _UM_MT_DECL              struct _thread *tp = _thread ();
#else
#define _UM_DEFAULT_REGULAR_HEAP _um_regular_heap
#define _UM_DEFAULT_TILED_HEAP   _um_tiled_heap
#define _UM_MT_DECL
#endif

/* Note that _um_regular_heap is declared in <umalloc.h>.
   _um_tiled_heap isn't, so we declare it here. */
...
{
#if defined (__MT__)
  _rmutex_checked_request (&h->rsem, _FMR_IGNINT);
#endif
}
...
{
#if defined (__MT__)
  _rmutex_checked_release (&h->rsem);
#endif
}
